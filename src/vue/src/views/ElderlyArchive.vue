      ...
      <div class="elderly-archive__hero-inner elderly-archive__hero-inner--filter">
        ...
        <div class="elderly-archive__filter-meta">
          ...
          <span v-if="filterNursingLevel" class="elderly-archive__filter-tag">
            <el-tag closable size="small" type="primary" @close="filterNursingLevel='';applyFilters()">{{ filterNursingLevel }}</el-tag>
          </span>
...
export default {
  ...
  methods: {
    ...
    /** 把库中历史中文值归一化为 LEVEL_x 编码 */
    normalizeNursingLevel(level) {
      const map = {
        '特别护理': 'LEVEL_1',
        '重点护理': 'LEVEL_2',
        '一般护理': 'LEVEL_3',
    ...
    applyFilters() {
      ...
      if (this.filterNursingLevel) {
        result = result.filter(e => this.normalizeNursingLevel(e.nursingLevel) === this.filterNursingLevel)
      }
    ...
    openEdit(row) {
      ...
      // 旧中文护理等级归一化为 LEVEL_x，保证下拉框正常回显，保存后也统一为新口径
      this.form.nursingLevel = this.normalizeNursingLevel(this.form.nursingLevel)
      this.dlgVisible = true
    },
    submitElderly() {
